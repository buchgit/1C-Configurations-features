...
|Функция ДвиженияВСтруктуру(Движения)|о.м. ОбщегоНазначенияБЗК|Преобразует коллекцию движений в структуру|

#Структуры и массивы

|Функция ЗначенияСовпадают(Значение1, Значение2)|о.м. ОбщегоНазначенияБЗК|Сравнивает значения на равенство. Работает для массивов|

#Формы, объекты

|Функция ЗначениеРеквизитаПоПутиКДанным(Объект, ПутьКДанным) |о.м. ОбщегоНазначенияБЗК|Получение значения свойства объекта по полному пути к данным|
|Функция НайтиСтрокиТаблицыФормы(ТаблицаРеквизит, ДанныеСтроки)|о.м. ОбщегоНазначенияБЗК||
|Функция НайтиСтрокуДереваРекурсивно(НаборСтрок, ДанныеСтроки, Найденные = Неопределено)|о.м. ОбщегоНазначенияБЗК||

#Сохранение состояния формы

|Функция ЗапомнитьВыделенныеСтроки(Форма, ТаблицаФормы, КлючевыеКолонки) |о.м. ОбщегоНазначенияБЗК|Запоминает выделенные строки таблицы формы|
|Процедура ВосстановитьВыделенныеСтроки(Форма, ТаблицаФормы, ВыделенныеСтроки)|о.м. ОбщегоНазначенияБЗК||

Свойства объектов

|Функция ЗначениеСвойства(Объект, ПутьКСвойству)|о.м. ОбщегоНазначенияБЗК|Получает значение свойства. Если свойство отсутствует, то возвращает Неопределено|
|Функция ЗначенияСвойств(Объект, ИменаСвойств) |о.м. ОбщегоНазначенияБЗК||

#Даты, периоды

|Функция НаименованиеПериода(НачалоПериода, ОкончаниеПериода)|о.м. ОбщегоНазначенияБЗК|Возвращает представление периода|
|Функция МассивДатИзПериода(ДатаНачала, ДатаОкончания)|о.м. ОбщегоНазначенияБЗК||
|Функция МассивДатИзПериодов(Периоды)|о.м. ОбщегоНазначенияБЗК||
|Функция ПериодыИзМассиваДат(МассивДат)|о.м. ОбщегоНазначенияБЗК|Возвращает массив периодов на основании массива дней|
|Функция КоличествоДнейВПериоде(ДатаНачала, ДатаОкончания) |о.м. ОбщегоНазначенияБЗК||
|Функция КоличествоДнейДоДаты(Дата) |о.м. ОбщегоНазначенияБЗК|Возвращает количество календарных дней оставшихся до даты|
|Функция СкопироватьДокументБезБизнесЛогики(ДокументОбъект)|о.м. ОбщегоНазначенияБЗК|Копирование документа с т.ч.|
|Функция РазмерПериода(Знач ДатаНачала, Знач ДатаОкончания)|о.м. ОбщегоНазначенияБЗК|Рассчитывает количество лет, месяцев и дней в периоде|

#Регистры сведений

|Функция СтруктураПоИмениРегистраСведений(ИмяРегистра)|о.м. ОбщегоНазначенияБЗК|Возвращает структуру по имени регистра сведений с ключами, соответствующими структуре регистра|
|Функция ТаблицаЗначенийПоМетаданнымРегистра(МетаданныеРегистра, ДобавитьКолонкуПериод)|о.м. ОбщегоНазначенияБЗК||
Регламентные задания

|Процедура ВключитьПредопределенноеЗадание(МетаданныеЗадания, Расписание)|о.м. ОбщегоНазначенияБЗК|Включает выполнение предопределенного регламентированного задания по расписанию|
|Процедура ОтключитьПредопределенноеЗадание(МетаданныеЗадания) |о.м. ОбщегоНазначенияБЗК||
